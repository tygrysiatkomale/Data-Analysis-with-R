...
ages <- c(22, 25, 26)
weights <- c(150, 165, 120)
sexs <- c('M', 'M', 'F')

df <- data.frame(names, ages, weights, sexs)
df
summary
```
...
```{r}
colMeans(subset(df, subset = hp > 100 & wt > 2.5)["mpg"])
```
...
```{r}
subset(mtcars, rownames(mtcars) == "Hornet Sportabout", select = mpg)
